...
transformed parameters {
  ...
  for (i in 1:r) {
    ...
    pi_param[, i] = X_train[i] * beta_pi[, i] + phi_pi[idx_train_er, i];
  }

  delta = exp(cholesky_decompose(corr_delta)' * Z_delta);
}
...
generated quantities {
  ...
  for (i in 1:r) {
    ...
    pi_param_full[, i] = X_full[i] * beta_pi[, i] + phi_pi[, i];
  }
