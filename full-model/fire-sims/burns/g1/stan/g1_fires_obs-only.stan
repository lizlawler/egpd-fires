...
transformed parameters {
  ...
  vector<lower = 0>[N_tb_obs] kappa;
  vector<lower = 0>[N_tb_obs] nu;
  vector<lower = 0>[N_tb_obs] xi;
  vector<lower = 0>[N_tb_obs] sigma;
  ...
  for (i in 1:r) {
    reg_kappa[, i] = X_tb[i] * beta_kappa[, i] + phi_kappa[tb_idx, i];
    reg_nu[, i] = X_tb[i] * beta_nu[, i] + phi_nu[tb_idx, i];
    reg_xi[, i] = X_tb[i] * beta_xi[, i] + phi_xi[tb_idx, i];
  }

  kappa = exp(to_vector(reg_kappa))[ii_tb_all][ii_tb_obs];
  nu = exp(to_vector(reg_nu))[ii_tb_all][ii_tb_obs];
  xi = exp(to_vector(reg_xi))[ii_tb_all][ii_tb_obs];
  sigma = nu ./ (1 + xi);
}
...
model {
  ...
  // likelihood
  for (k in 1:N_tb_obs) {
    target += egpd_g1_lpdf(y_tb_obs[k] | sigma[k], xi[k], kappa[k]);
  }
}
...
generated quantities {
  ...
  for (k in 1:N_tb_obs) {
    train_loglik[k] = egpd_g1_lpdf(y_tb_obs[k] | sigma[k], xi[k], kappa[k]);
  }
