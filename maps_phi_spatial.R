...
er_map <- ecoregions_geom %>%
  ggplot() +
  geom_sf(size = .2, fill = "white") +
  geom_sf(data = ecoregions_geom,
          aes(fill = NA_L2CODE), alpha = 0.6, lwd = 0, inherit.aes = FALSE, show.legend = FALSE) +
  # scale_fill_manual(values = l3_palette(length(unique(ecoregions_geom$NA_L2NAME)))) +
  geom_sf(data = ecoregions_geom %>% group_by(NA_L1CODE) %>% summarise(),
          fill = "transparent", size = 0.9, color = "gray20", inherit.aes = FALSE, show.legend = FALSE) +
  theme_void() +
  coord_sf(ndiscr = FALSE)
  # theme(panel.grid.major = element_line(colour = "lightgrey"))

ggsave("er_map.png", dpi = 300, type = "cairo")
