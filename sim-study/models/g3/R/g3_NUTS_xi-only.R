...
# generate toy data
source("./sim-study/models/g3/R/g3_data_xi-only.R")

# run sampling
egpd_init <- stan_model('./sim-study/models/g3/stan/g3_xi-only.stan')
...
saveRDS(egpd_fit, file = "./sim-study/models/g3/stan-fits/g3_xi-only.RDS")
...
post_effects_xi <- ggplot(xi_full, aes(x=linear, y=effect, group = region)) + 
  ...
  facet_grid(type ~ .)
ggsave(paste0('./sim-study/figures/g3/effects/g3_post-effects-xi_',
...
ggsave(paste0('./sim-study/figures/g3/maps/g3_phi-map-xi_',
              format(as.POSIXlt(Sys.time(), "America/Denver"), "%d%b%Y_%H%M"),
              ".pdf"),
...
MCMCtrace(egpd_fit, params = c("beta_xi", "phi_xi", "rho1_xi", "rho2_xi"),
          ...
          open_pdf = FALSE,
          filename = paste0('./sim-study/figures/g3/trace/g3_trace-xi_',
