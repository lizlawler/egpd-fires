...
generated quantities {
  ...
  if (max(y_train_obs) < 30) {
    ...
    vector[N_tb_obs] mu_train = exp(to_vector(ri_matrix))[ii_tb_all][ii_tb_obs];
    vector[N_tb_obs] sigma_train = to_vector(reg_full[1])[ii_tb_all][ii_tb_obs];
    vector[N_hold_obs] mu_hold = exp(to_vector(ri_matrix))[ii_hold_all][ii_hold_obs];
    vector[N_hold_obs] sigma_hold = to_vector(reg_full[1])[ii_hold_all][ii_hold_obs];    
