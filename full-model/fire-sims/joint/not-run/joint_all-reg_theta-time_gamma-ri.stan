...
transformed data {
  ...
  // ordering: 1=lambda, 2=kappa, 3 = sigma, 4=pi, 5=delta, 6=xi, 7,8 = gammas
}
parameters {
  array[N_tb_mis] real<lower=y_min> y_train_burn_mis;
  matrix[R, C-S] Z; // ordering: 1 = pi, 2 = delta, 3 = xi, 5,6 = gammas
  ...
  vector[T_all] theta_init; // initialization of shared random effect, varying in time
  real<lower=0> eps; // noise for AR(1) prior on theta
}
transformed parameters {
  array[N_tb_all] real<lower=y_min> y_train_burn;
  vector<lower = 0>[R] delta;
  array[2] vector[R] gamma; // scaling parameter; 1 = kappa, 2 = sigma
  matrix[T_train, R] lambda;
  vector[R] pi_prob;
  array[S] matrix[T_all, R] phi;
  array[2] matrix[T_train, R] reg; // 1=kappa, 2 = sigma
  ...
  for (s in 2:S) {
    ...
    for (r in 1:R) {
      ...
      reg[s-1][, r] = X_train_burn[r] * beta_burn[s-1][, r] + phi[s][idx_train_er, r] + gamma[s-1][r] * theta[idx_train_er];
    }
  }
}
model {
...
generated quantities {
  ...
  for (s in 2:S) {
    for (r in 1:R) {
      reg_full[s-1][, r] = X_full_burn[r] * beta_burn[s-1][, r] + phi[s][, r] + gamma[s-1][r] * theta;
    }
  }
