functions {
  ...
  vector forecast_rng(int pred, real sigma, real xi, real kappa) {
    vector[pred] forecast;
    real u;
    real u_adj;
    ...
    for(p in 1:pred) {
      u = uniform_rng(0, 1);
      u_adj = u * (1-cst) + cst;
      forecast[p] = (sigma/xi) * ((1-u_adj^(1/kappa))^-xi) - 1;
    }
    return forecast;
  }
  ...
  real twCRPS(real y, vector forecast, real delta, real w_mean, real w_sd) {
    ...
    summand = 0;
    for(i in 1:n) {
      summand += (forecast[i] - step(forecast[i] - y))^2 * normal_cdf(forecast[i], w_mean, w_sd);
    }
    score = summand * delta;
    return score;
  }
...
generated quantities {
  ...
  real holdout_twcrps[N_hold_obs];
  ...
  if (max(y_train_obs) < 50) { // condition determines if the data read in are the sqrt or original burn areas
    ...
    for (n in 1:N_tb_obs) {
      train_loglik[n] = egpd_lpdf(y_train_obs[n] | sigma_train[n], xi_train[n], kappa_train[n]) + log(0.5) - log(y_train_obs[n]);
    }
    // holdout scores
    for (n in 1:N_hold_obs) {
      // log-likelihood
      holdout_loglik[n] = egpd_lpdf(y_hold_obs[n] | sigma_hold[n], xi_hold[n], kappa_hold[n]) + log(0.5) - log(y_hold_obs[n]);
      // twCRPS
      holdout_twcrps[n] = twCRPS(y_hold_obs[n], forecast_rng(n_pred, sigma_hold[n], xi_hold[n], kappa_hold[n]), delta, sqrt(21), 3);
    }
  }
  else {
    // training log-likelihood
    ...
    for (n in 1:N_hold_obs) {
      ...
      // twCRPS 
      holdout_twcrps[n] = twCRPS(y_hold_obs[n], forecast_rng(n_pred, sigma_hold[n], xi_hold[n], kappa_hold[n]), delta, 21, 9);
    }
  }
}
