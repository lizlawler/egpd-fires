...
betas_delta <- matrix(runif(p*r, -.25, .25), p, r)
reg_delta <- matrix(NA, r, t)
reg_nu <- matrix(NA, r, t)
reg_xi <- matrix(NA, r, t)

for(i in 1:r) {
  reg_delta[i,] <- X_full[i, , ] %*% betas_delta[, i]
  reg_nu[i,] <- X_full[i, , ] %*% betas_nu[, i]/5 + phi_mat_nu[,i]/15
...
write_rds(toy_data, 'sim-study/models/g3/data/g3_nu-only.rds')