...
parameters {
  ...
  vector<lower=rho1, upper = 1>[C] rho_sum; // 1 = nu, 2 = kappa, 3 = xi, 4 = delta
}
transformed parameters {
  ...
  array[C] corr_matrix[R] corr;  // 1 = nu, 2 = kappa, 3 = xi, 4 = delta
...
model {
  vector[N_ts_all] nu = exp(to_vector(reg[1]))[ii_ts_all];
  vector[N_ts_all] kappa = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_ts_all];
  vector[N_ts_all] xi = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_ts_all];
  vector[N_ts_all] delta = exp(to_vector(ri_matrix[3][idx_train_er,]))[ii_ts_all];
  vector[N_ts_all] sigma = nu ./ (1+xi);
...
generated quantities {
  ...
  // training scores
  for (n in 1:N_ts_obs) {
    real nu_train = exp(to_vector(reg_full[1][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
    real kappa_train = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
    real xi_train = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
    real delta_train = exp(to_vector(ri_matrix[3][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
    ...
    vector[n_int] pred_probs_train = prob_forecast(n_int, int_pts, y_min, 
                                            sigma_train, xi_train, delta_train, kappa_train);
  ...
  for (n in 1:N_hold_obs) {
    ...
    vector[n_int] pred_probs_hold = prob_forecast(n_int, int_pts, y_min, 
                                          sigma_hold, xi_hold, delta_hold, kappa_hold);
