...
model {
  ...
  vector[N_tb_all] xi = (inv_logit(to_vector(ri_matrix[2][idx_train_er,]))[ii_tb_all]) * 1.5 - 0.25;
...
generated quantities {
  ...
  for (n in 1:N_tb_obs) {
    ...
    real xi_train = (inv_logit(to_vector(ri_matrix[2][idx_train_er,]))[ii_tb_all][ii_tb_obs][n]) * 1.5 - 0.25;
  ...
  for (n in 1:N_hold_obs) {
    ...
    real xi_hold = (inv_logit(to_vector(ri_matrix[2]))[ii_hold_all][ii_hold_obs][n]) * 1.5 - 0.25;
  ...
  for (r in 1:R) {
    for (t in 1:T_all) {
      ...
      real xi = inv_logit(ri_init[2][r]) * 1.5 - 0.25;
