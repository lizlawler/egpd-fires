...
transformed parameters {
  array[N_ts_all] real<lower=1> y_train;
  matrix[T_all, R] phi;
...
model {
  vector[N_ts_all] mu = to_vector(reg)[ii_ts_all];
...
generated quantities {
  ...
  // training scores
  for (n in 1:N_ts_obs) {
    real mu_train = exp(to_vector(reg_full[idx_train_er,]))[ii_ts_all][ii_ts_obs][n];

    train_loglik[n] = lognorm_trunc_lpdf(y_train_obs[n] | y_min, mu_train, sigma);
    // forecasting then twCRPS, on training dataset
    vector[n_int] pred_probs_train = prob_forecast(n_int, int_pts, y_min, mu_train, sigma);
  ...
  for (n in 1:N_hold_obs) {
    ...
    vector[n_int] pred_probs_hold = prob_forecast(n_int, int_pts, y_min, mu_hold, sigma);
