...
transformed data {
  ...
  // ordering: 1=lambda, 2=kappa, 3=pi, 4=delta, 5=kappa, 6=xi, 7=theta, 8 = gamma
}
parameters {
  array[N_tb_mis] real<lower=y_min> y_train_burn_mis;
  matrix[R, C-S] Z; // ordering: 1 = pi, 2 = delta, 3 = kappa, 4 = xi, 5=theta, 6 = gamma
...
transformed parameters {
  ...
  vector[R] gamma; // linking parameter, varying by region
  ...
  theta = cholesky_decompose(corr[7])' * Z[,5];
  ...
  for (r in 1:R) {
    ...
    reg[, r] = X_train_burn[r] * beta_burn[, r] + phi[2][idx_train_er, r] + gamma[r] * theta[r];
  }
}
model {
...
generated quantities {
  ...
  for (r in 1:R) {
    reg_full[, r] = X_full_burn[r] * beta_burn[, r] + phi[2][, r] + gamma[r] * theta[r];
  }
  ...
  // training scores
  for (n in 1:N_tb_obs) {
    real sigma_train = exp(to_vector(reg_full))[ii_tb_all][ii_tb_obs][n];
