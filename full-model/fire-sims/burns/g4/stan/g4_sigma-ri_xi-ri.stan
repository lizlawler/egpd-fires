...
parameters {
  ...
  array[C] vector<lower=0, upper=1>[2] rho; // 1 = kappa, 2 = sigma, 3 = xi, 4 = gamma
}
transformed parameters {
  ...
  array[C] matrix[R, R] corr; // 1 = kappa, 2= sigma, 3 = xi, 4 = gamma
  ...
  for (s in 1:S) {
    ...
    // regression for gamma, sigma, and xi
    for (r in 1:R) {
  ...
  sigma = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_tb_all];
