...
parameters {
  ...
  array[C] vector<lower=0, upper=1>[2] rho; // ordering: 1=nu, 2,3=kappas, 4 = xi
}
transformed parameters {
  ...
  array[C] matrix[R, R] corr; // ordering: 1=nu, 2,3=kappas, 4 = xi
  ...
  kappa1 = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_tb_all];
  kappa2 = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_tb_all];
  xi = exp(to_vector(ri_matrix[3][idx_train_er,]))[ii_tb_all];
  sigma = nu ./ (1 + xi);
}
model {
...
//   kappa1_train = exp(to_vector(ri_matrix[1]))[ii_tb_all][ii_tb_obs];
//   kappa2_train = exp(to_vector(ri_matrix[2]))[ii_tb_all][ii_tb_obs];
//   xi_train = exp(to_vector(ri_matrix[3]))[ii_tb_all][ii_tb_obs];
//   sigma_train = nu_train ./ (1 + xi_train);
//   
//   nu_hold = exp(to_vector(reg_full[1]))[ii_hold_all][ii_hold_obs];
//   kappa1_hold = exp(to_vector(ri_matrix[1]))[ii_hold_all][ii_hold_obs];
//   kappa2_hold = exp(to_vector(ri_matrix[2]))[ii_hold_all][ii_hold_obs];
//   xi_hold = exp(to_vector(ri_matrix[3]))[ii_hold_all][ii_hold_obs];
