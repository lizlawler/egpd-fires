...
transformed parameters {
  ...
  array[C] matrix[R, R] corr; // 1 = kappa, 2= sigma, 3 = xi, 4 = delta
  ...
  delta = exp(to_vector(ri_matrix[3][idx_train_er,]))[ii_tb_all];
}
model {
  Z[1] ~ std_normal();
  Z[2] ~ std_normal();
  ...
  to_vector(rho[1, ]) ~ beta(3, 4); // prior on rho1 for kappa, delta, sigma, and xi
  // to_vector(rho[2,]) ~ beta(1.5, 4); // prior on rho2 for kappa, delta, sigma, and xi
...
//   delta_train = exp(to_vector(ri_matrix[3]))[ii_tb_all][ii_tb_obs];
//   
...
//   delta_hold = exp(to_vector(ri_matrix[3]))[ii_hold_all][ii_hold_obs];
//   
