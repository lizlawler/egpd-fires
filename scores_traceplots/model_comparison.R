##############################################################################
##  This script produces tables of scores in the following manner:          ##
...
## load in sizes models run on ERC dataset ------------------------------------
...
score_files <- paste0("./scores_traceplots/extracted_scores/",
                      list.files("./scores_traceplots/extracted_scores/", 
                                 "erc"))
score_files <- score_files[!grepl("fwi", score_files)]
sizes_model_names <- basename(score_files) %>% 
  str_remove(., "_erc_\\d{2}\\w{3}2023_\\d{4}_scores.RDS")
...
g3_files <- paste0("./models/sizes/g3/csv_fits/",
                   list.files("./models/sizes/g3/csv_fits/", 
                              "nu"))
g3_name <- basename(g3_files[1]) %>% str_remove(., "_erc_\\d{2}\\w{3}2023_\\d{4}_1.csv")
g4_files <- paste0("./models/sizes/g4/csv_fits/",
                   list.files("./models/sizes/g4/csv_fits/", 
                              ".csv"))
g4_name <- basename(g4_files[1]) %>% str_remove(., "_erc_\\d{2}\\w{3}2023_\\d{4}_1.csv")
