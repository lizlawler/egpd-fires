...
transformed parameters {
  ...
  gamma = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_tb_all];
  sigma = nu ./ (1 + xi);
}
model {
...
//   gamma_train = exp(to_vector(ri_matrix[2]))[ii_tb_all][ii_tb_obs];
...
//   gamma_hold = exp(to_vector(ri_matrix[2]))[ii_hold_all][ii_hold_obs];
