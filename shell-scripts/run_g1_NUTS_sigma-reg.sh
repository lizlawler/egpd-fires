...
export modtype modname params suffix sttime
parentjob=$(sbatch --parsable $1 --job-name ${modname}_${suffix}_${params}_${sttime} \
--output="./full-model/output/%x_%j.txt" \
shell-scripts/call_sampler.sh)
sleep 1
sbatch --dependency=afterok:${parentjob} \
--job-name ${modname}_${suffix}_${params}_plots \
--output="./full-model/output/%x_%j.txt" \
