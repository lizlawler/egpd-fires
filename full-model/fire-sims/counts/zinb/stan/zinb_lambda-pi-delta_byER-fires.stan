...
model {
  // priors
  matrix[r, r] chol_delta = cholesky_decompose(corr_delta);
  vector[r] mu_delta = rep_vector(0, r);
  delta ~ multi_normal_cholesky(mu_delta, chol_delta);
    ...
    for (j in 1:t_tc) {
       if (y_tc[j, i] == 0) {
        target += log_sum_exp(bernoulli_logit_lpmf(1 | pi_param[j, i]),
                            ...
                            + neg_binomial_2_log_lpmf(y_tc[j, i] | lambda[j, i], delta[i]));
      } else {
    ...
    for (j in 1:t_hold) {
       if (y_hold[j, i] == 0) {
        holdout_loglik[j, i] = log_sum_exp(bernoulli_logit_lpmf(1 | pi_param_hold[j, i]),
                             ...
                             + neg_binomial_2_log_lpmf(y_hold[j, i] | lambda_hold[j, i], delta[i]));
      } else {
