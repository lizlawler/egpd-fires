...
# joint model tables
twcrps_full <- readRDS("~/research/egpd-fires/full-model/fire-sims/model_comparison/joint_twcrps.RDS")
...
total_ll <- readRDS("~/research/egpd-fires/full-model/fire-sims/model_comparison/joint_loglik.RDS")$total_ll
...
joint_scores <- joint_scores %>% 
  mutate(model = case_when(grepl("gamma-ri", model) ~ 'gamma-ri',
                           TRUE ~ 'gamma-cst'))

joint_scores %>% mutate_if(is.numeric, round, 3) %>% 
    mutate(model = case_when(model == 'gamma-ri' ~ "$\\gamma$-RI", 
                             TRUE ~ '$\\gamma$-cst')) %>% 
  rename("Model" = model, "Mean_tw" = twcrps_mean_diff, "SD_tw" = twcrps_sd_diff, "Mean_ll" = loglik_mean_diff, "SD_ll" = loglik_sd_diff) %>% 
  kable(., format='latex', align = 'c', booktabs=T, escape = F) %>% add_header_above(c(" "=1, "twCRPS" = 2, "Log-likelihood" = 2)) %>%
