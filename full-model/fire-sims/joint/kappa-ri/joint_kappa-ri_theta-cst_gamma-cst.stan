...
transformed data {
  ...
  //ordering of S: 1 = lambda, 2 = sigma, 3 = kappa
  int C = 6; // # of parameters with correlation (either regression or random intercept)
  // ordering: 1=lambda, 2=sigma, 3=pi, 4=delta, 5=kappa, 6=xi
}
parameters {
  array[N_tb_mis] real<lower=y_min> y_train_burn_mis;
  matrix[R, C-S] Z; // ordering: 1 = pi, 2 = delta, 3 = kappa, 4 = xi
...
transformed parameters {
  ...
  for (r in 1:R) {
    ...
    reg[, r] = X_train_burn[r] * beta_burn[, r] + phi[2][idx_train_er, r] + gamma * theta;
  }
}
model {
  theta ~ normal(0, 5);
