...
for(i in seq_along(burn_names)) {
  # burn_loglik <- get(burn_names[i])[1:2]
  holdout_loglik_burns[[i]] <- get(burn_names[i])[[2]]$generated_quantities %>%
    apply(., c(1,2), sum) %>%
    as_tibble() %>%
  ...
  train_loglik_burns[[i]] <- get(burn_names[i])[[1]]$generated_quantities %>%
    apply(., c(1,2), sum) %>%
    as_tibble() %>%
    rowid_to_column(var = "iter") %>%
    mutate(mean_ax_cns = rowMeans(select(., !iter))) %>%
...
# 
ll_boxplot_0.81_df <- ll_full_0.81 %>% pivot_longer(cols = c("test", "train"), names_to = "dataset") %>%
  filter(chain == 'mean_ax_cns')


std_0.81_df <- ll_full_0.81 %>% pivot_longer(cols = c("test", "train"), names_to = "dataset") %>% 
  mutate(total_obs = case_when(dataset == "test" ~ 3314,
                               dataset == "train" ~ 8018),
         std_value = value/total_obs)
  
ll_box_0.81_test <- std_0.81_df %>% filter(chain == "mean_ax_cns", dataset == "test") %>% ggplot(aes(model, std_value, color = dataset)) + geom_boxplot() + theme_minimal()
ll_box_0.81_train <- std_0.81_df %>% filter(chain == "mean_ax_cns", dataset == "train") %>% ggplot(aes(model, std_value, color = dataset)) + geom_boxplot() + theme_minimal()
ll_box_0.81 <- std_0.81_df %>% filter(chain == "mean_ax_cns") %>% ggplot(aes(model, std_value, color = dataset)) + geom_boxplot() + theme_minimal()


std_0.9_df <- ll_full_0.90 %>% pivot_longer(cols = c("test", "train"), names_to = "dataset") %>% 
  mutate(total_obs = case_when(dataset == "test" ~ 3314,
                               dataset == "train" ~ 8018),
         std_value = value/total_obs)
ll_box_0.9_test <- std_0.9_df %>% filter(chain == "mean_ax_cns", dataset == "test") %>% ggplot(aes(model, std_value, color = dataset)) + geom_boxplot() + theme_minimal()
ll_box_0.9_train <- std_0.9_df %>% filter(chain == "mean_ax_cns", dataset == "train") %>% ggplot(aes(model, std_value, color = dataset)) + geom_boxplot() + theme_minimal()
ll_box_0.9 <- std_0.9_df %>% filter(chain == "mean_ax_cns") %>% ggplot(aes(model, std_value, color = dataset)) + geom_boxplot() + theme_minimal()
...
ll_ranked_test_0.9 <- ll_full_0.90 %>% filter(chain == 'mean_ax_cns') %>%
  group_by(model) %>%
  summarize(mean_test = mean(test),
            sd_test = sd(test)) %>%
  arrange(-mean_test)
top_mod_test_0.90 <- ll_ranked_test_0.9$model[1]
# 
ll_comp_test_0.9 <- ll_full_0.90 %>% filter(chain == 'mean_ax_cns') %>%
  select(c(1, 3:4)) %>% pivot_wider(names_from = model, values_from = test) %>%
...
ll_ranked_train_0.81 <- ll_full_0.81 %>% 
  select(-test) %>% 
  filter(chain != "mean_ax_cns", train != Inf) %>% 
  group_by(model) %>% 
  summarize(mean_train = mean(train), sd_train = sd(train)) %>% 
  arrange(-mean_train)
top_mod_train_0.81 <- ll_ranked_train_0.81$model[1]
# 
ll_comp_train_0.81 <- ll_full_0.81 %>% 
  select(-test) %>%
  filter(chain != "mean_ax_cns", train != Inf) %>% 
  group_by(model, iter) %>% 
...
ll_ranked_train_0.90 <- ll_full_0.90 %>% 
  select(-test) %>% 
  filter(chain != "mean_ax_cns", train != Inf) %>% 
  group_by(model) %>% 
  summarize(mean_train = mean(train), sd_train = sd(train)) %>% 
...
# 
ll_comp_train_0.90 <- ll_full_0.90 %>% 
  select(-test) %>%
  filter(chain != "mean_ax_cns", train != Inf) %>% 
  group_by(model, iter) %>% 
  summarize(mean = mean(train)) %>% 
...
# twcrps calculations ---------
holdout_twcrps_burns <- vector("list", nfits)
for(i in seq_along(burn_names)) {
  # burn_loglik <- get(burn_names[i])[1:2]
  holdout_twcrps_burns[[i]] <- get(burn_names[i])[[3]]$generated_quantities %>%
    apply(., c(1,2), sum) %>%
    as_tibble() %>%
    rowid_to_column(var = "iter") %>%
    mutate(mean_ax_cns = rowMeans(select(., !iter))) %>%
...
holdout_twcrps_b <- bind_rows(holdout_twcrps_burns)
holdout_twcrps_b
...
temp_df <- `g1_sqrt_xi-ri_0.81`$betas %>%
  mutate(dataset = 
...
beta_vars <- variables(`g1_sqrt_xi-ri_0.81`[[1]]) %>% as_tibble() %>% separate(value, into = c("param", "covar", "region"), sep = ",") %>%
  mutate(param = as.numeric(str_remove(param, "beta\\[")),
         region = as.numeric(str_remove(region, "\\]")),
         covar = as.numeric(covar))
nu_beta <- beta_vars %>% filter(param == 2) %>% select(-param) %>% arrange(covar, region) %>% pivot_wider(names_from = region, values_from = value) %>%
  select(-covar) %>% as.matrix()

beta_vars <- `g1_sqrt_xi-ri_0.81`$betas %>% apply(., c(1,3), median) %>% as_tibble() %>% pivot_longer(everything(), names_to = "combo", values_to = "value")

beta_vars <- beta_vars %>% separate(combo, into = c("param", "covar", "region"), sep = ",")
