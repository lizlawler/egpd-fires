...
#                 list.files("full-model/fire-sims/joint/csv-fits/", pattern = params))
...
draws_path <- paste0("full-model/figures/paper/mcmc_draws/", params, "_", iter, "iter/")
dir.create(path = draws_path, recursive = TRUE)

burn_pred_draws <- read_cmdstan_csv(files, variables = "burn_pred")$generated_quantities
...
print("MCMC draws from burn predictions have been successfully written to disk")
