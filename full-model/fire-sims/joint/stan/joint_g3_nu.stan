...
transformed data {
  ...
  //ordering of S: 1 = lambda, 2 = nu
  int C = 7; // # of parameters with correlation (either regression or random intercept)
  // ordering: 1=lambda, 2=nu, 3=pi, 4=delta, 5 = xi, 6 = zeta, 7 = gamma
}
parameters {
  array[N_tb_mis] real<lower=y_min> y_train_burn_mis;
  matrix[R, C-S] Z; // ordering: 1 = pi, 2 = delta, 3 = xi, 4=zeta, 5 = gamma
...
model {
  vector[N_tb_all] nu = exp(to_vector(reg))[ii_tb_all];
  vector[N_tb_all] xi = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_tb_all];
  vector[N_tb_all] zeta = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_tb_all];
  ...
  // burn likelihood
  for (n in 1:N_tb_all) {
    target += egpd_trunc_lpdf(y_train_burn[n] | y_min, sigma[n], xi[n], zeta[n]);
  }
...
generated quantities {
  ...
  // training scores
  for (n in 1:N_tb_obs) {
    real nu_train = exp(to_vector(reg_full[idx_train_er,]))[ii_tb_all][ii_tb_obs][n];
  ...
  for (n in 1:N_hold_obs) {
    real nu_hold = exp(to_vector(reg_full))[ii_hold_all][ii_hold_obs][n];
    real xi_hold = exp(to_vector(ri_matrix[1]))[ii_hold_all][ii_hold_obs][n];
    real zeta_hold = exp(to_vector(ri_matrix[2]))[ii_hold_all][ii_hold_obs][n];
    real sigma_hold = nu_hold / (1 + xi_hold);
    
    // log-likelihood
    holdout_loglik_burn[n] = egpd_trunc_lpdf(y_hold_burn_obs[n] | y_min, sigma_hold, xi_hold, zeta_hold);
      // forecasting then twCRPS, on holdout dataset
    vector[n_int] pred_probs_hold = prob_forecast(n_int, int_pts_holdout, y_min, 
                                          sigma_hold, xi_hold, zeta_hold);
  ...
  for (r in 1:R) {
    for (t in 1:T_all) {
      ...
      real nu = exp(reg_full[t, r]);
      real xi = exp(ri_init[1][r]); // zeta = delta in G3 function, since delta is used in ZINB component
      real zeta = exp(ri_init[2][r]);
      real sigma = nu ./ (1 + xi);
      for (i in 1:500) {
        ...
        if (count_draw == 0) {
          ...
          burn_draws[i] = sum(egpd_rng(count_draw, y_min, sigma, xi, zeta));
        }
      }
