...
model {
  ...
  vector[N_ts_all] nu = exp(to_vector(reg[2]))[ii_ts_all];
  vector[N_ts_all] xi = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_ts_all];
  vector[N_ts_all] delta = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_ts_all];
...
generated quantities {
  ...
  for (n in 1:N_ts_obs) {
    ...
    real nu_train = exp(to_vector(reg_full[2][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
    real xi_train = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
    real delta_train = exp(to_vector(ri_matrix[2][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
