...
data {
  ...
  real<lower = 0, upper = 1> zero_prob[R]; // empirical probability of extra zeroes
...
model {
  ...
  for (i in 1:R) {
    for (j in 1:T) {
       if (y[j, i] == 0) {
        target += log_sum_exp(bernoulli_lpmf(1 | zero_prob[i]),
                            bernoulli_lpmf(0 | zero_prob[i])
                            + poisson_lpmf(y[j, i] | lambda[j, i]));
      } else {
        target += bernoulli_lpmf(0 | zero_prob[i])
                + poisson_lpmf(y[j, i] | lambda[j, i]);
      }
    }
  }
