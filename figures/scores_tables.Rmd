...
sizes_model_scores_table <- sizes_model_scores %>% 
  mutate(kappa = case_when(grepl('sigma-ri', model) ~ 'reg',
         ...
         xi = case_when(grepl('xi-ri', model) ~ 'ri',
                        .default = NA),
         time = case_when(grepl('g4', model) ~ 636903, # grab sampling elapsed time from console output of each model
                          grepl('g3', model) ~ 332067,
                          grepl('g2', model) ~ 31936,
                          grepl('g1', model) ~ 23195,
                          grepl('cst', model) ~ 19096,
                          .default = 16157),
