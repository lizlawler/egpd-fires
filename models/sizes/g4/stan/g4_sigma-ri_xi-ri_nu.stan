...
parameters {
  ...
  matrix[R, 3] Z; // 1=nu, 2 = xi, 3=delta
...
model {
  ...
  vector[N_ts_all] nu = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_ts_all];
...
generated quantities {
  ...
  for (n in 1:N_ts_obs) {
    ...
    real nu_train = exp(to_vector(ri_matrix[1][idx_train_er,]))[ii_ts_all][ii_ts_obs][n];
