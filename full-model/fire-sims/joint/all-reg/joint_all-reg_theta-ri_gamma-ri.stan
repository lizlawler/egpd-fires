...
transformed data {
  ...
  int C = 9; // # of parameters with correlation (either regression or random intercept)
}
parameters {
  array[N_tb_mis] real<lower=y_min> y_train_burn_mis;
  matrix[R, C-S] Z; // ordering: 1 = pi, 2 = delta, 3 = xi, 4 = theta, 5,6 = gammas
...
transformed parameters {
  ...
  vector[R] theta; //shared random effect, varying by region
  ...
  array[2] vector[R] gamma; // used in burn link; 1 = kappa, 2 = sigma
  ...
  theta = cholesky_decompose(corr[7])' * Z[,4];
  ...
  for (s in 2:S) {
    gamma[s-1] = cholesky_decompose(corr[s+(C-S)])' * Z[,s+(C-2*S)];
    for (r in 1:R) {
      lambda[, r] = X_train_count[r] * beta_count[, r] + phi[1][idx_train_er, r] + area_offset[r] + theta[r];
      reg[s-1][, r] = X_train_burn[r] * beta_burn[s-1][, r] + phi[s][idx_train_er, r] + gamma[s-1][r] * theta[r];
    }
  }
}
model {
...
generated quantities {
  ...
  for (s in 2:S) {
    for (r in 1:R) {
      reg_full[s-1][, r] = X_full_burn[r] * beta_burn[s-1][, r] + phi[s][, r] + gamma[s-1][r] * theta[r];
    }
  }
  ...
  for (r in 1:R) {
    vector[T_hold] lambda_hold = (X_full_count[r] * beta_count[, r] + phi[1][, r])[idx_hold_er] + area_offset[r] + theta[r];
    for (t in 1:T_hold) {
